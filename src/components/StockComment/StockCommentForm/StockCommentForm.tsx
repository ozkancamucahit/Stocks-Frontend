...
interface Props  {
    symbol :string;
    handleComment : (e :CommentInputForms) => void;
};

export type CommentInputForms ={
  title :string;
  content :string;
};
...
const StockCommentForm = ({symbol, handleComment}: Props) => {
  ...
  const {register, handleSubmit, formState: {errors}} = useForm<CommentInputForms>({resolver: yupResolver(validation)});
